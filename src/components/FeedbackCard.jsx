...
const FeedbackCard = ({ data, onNext }) => {
  if (!data) return null;
  ...
  return (
    <div className={`relative p-8 rounded-[2rem] w-full h-full md:h-auto max-w-sm flex flex-col items-center justify-center text-center shadow-2xl transition-all duration-500 animate-in fade-in zoom-in-95 overflow-hidden border ${!isSuccess ? 'bg-red-950/90 border-red-500 shadow-red-900/50' : ''}`}>
      {!isSuccess ? (
        <>
          <XCircle className="w-24 h-24 text-red-500 mb-6 drop-shadow-[0_0_15px_rgba(239,68,68,0.5)]" />
          <h2 className="text-3xl font-black text-white mb-2 tracking-tight">ERROR</h2>
          <p className="text-red-400 font-bold tracking-widest uppercase mb-6 text-sm">{data.message}</p>
        </>
      ) : null}
      ...
      {isSuccess && data.full_name && (
        <div className={`relative z-10 mt-6 bg-slate-900/50 border ${isVip ? 'border-yellow-500/30 shadow-[inset_0_0_20px_rgba(234,179,8,0.1)]' : 'border-slate-700/50 shadow-inner'} w-full rounded-2xl py-5 px-4 backdrop-blur-md`}>
          ...
          <p className={`text-2xl font-bold ${isVip ? 'text-yellow-100' : 'bg-gradient-to-r from-white to-slate-300 bg-clip-text text-transparent'}`}>{data.full_name}</p>
          ...
          {data.blockName && (
            <div className={`mt-3 py-2 border-y ${isVip ? 'border-yellow-500/20 bg-yellow-950/20' : 'border-slate-700/50 bg-slate-800/20'}`}>
              ...
              <p className={`text-sm font-black tracking-wider ${isVip ? 'text-yellow-400' : 'text-indigo-400'}`}>{data.blockName}</p>
            </div>
          )}
          ...
          {data.entered_at && (
            <div className="mt-3 pt-2 flex flex-col items-center">
              ...
              <p className="text-lg font-mono font-medium text-emerald-400 tracking-widest">{new Date(data.entered_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', second: '2-digit' })}</p>
            </div>
          )}
        </div>
      )}
      ...
      {!isSuccess && data.message && (
        <div className="relative z-10 mt-4 mb-2 bg-red-950/30 border border-red-500/20 w-full rounded-xl py-3 px-4">
          <p className="text-sm font-medium text-red-200">{data.message}</p>
        </div>
      )}
      ...
      <div className="relative z-10 mt-8 w-full border-t border-slate-800 pt-6">
        ...
        <p className="font-mono font-medium text-slate-300 bg-slate-950/80 border border-slate-800 py-3 rounded-xl break-all px-4 text-sm tracking-widest">{data.pin_number || data.vip_code || data.fac_code || 'N/A'}</p>
      </div>
